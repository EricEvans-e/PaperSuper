...
const buildVisualHtmlPrompt = ({
  ...
  [
    ...
    "Analyze the paper concept below and create a self-contained interactive teaching visualization for it.",
    ...
    "Goal: Create a mechanism-first visualization based on the selected paper passage, not a generic flowchart.",
    "Layout: compact right-side panel, no hero banner, no oversized icons, no decorative dashboard.",
    "Required parts:",
    "- One dominant principle/mechanism drawing using SVG or DOM shapes that directly illustrates the selected passage.",
    "- 1 to 3 range sliders relevant to the mechanism.",
    "- 2 to 3 live indicators showing computed results.",
    ...
    "Choose an animation style that fits the actual mechanism: tensor reshape, convolution kernel sliding, attention matrix weighting, module assembly, state transition, geometric transform, memory transfer, equation transformation, or any other structure that matches the paper content.",
    "Use Simplified Chinese for UI text, but keep technical terms when clearer.",
    "Keep all CSS scoped to this fragment. Avoid broad body styles except margin/font/background if needed.",
    "",
    `Paper title: ${paper.title || "Untitled PDF"}`,
    `Selected page: ${activeContext.pageNumber || "unknown"}`,
    "Selected passage (this is the PRIMARY source — base your visualization on this content):",
    activeContext.text,
    "",
    "Paper context excerpt:",
    paperContext,
    "",
    "Structured data for reference only (use for additional context, but always prioritize the selected passage above):",
    JSON.stringify({
      title: spec.title,
...
const buildFacetSvgPrompt = ({
  ...
  [
    ...
    "",
    "## COLOR AND READABILITY (CRITICAL)",
    "",
    "- The FIRST element inside <svg> MUST be a background <rect> covering the full viewBox, e.g. <rect width='960' height='540' fill='#f8f9fa'/>.",
    "- Background fill: use white #ffffff, light gray #f8f9fa, or light blue #f0f4f8. Do NOT use dark backgrounds.",
    "- All <text> elements MUST have an explicit fill attribute: use dark colors like #1a1a2e, #333, or #1f2937 for text on light backgrounds.",
    "- Region/box fills: use light pastel fills (#e8f4fd, #fef3c7, #d1fae5, #ede9fe) with darker borders (#3b82f6, #d97706, #059669, #7c3aed).",
    "- If a region has a dark fill (e.g. #1e3a5f), use LIGHT text inside it (#ffffff or #e5e7eb).",
    "- Minimum font-size: 13px for labels, 15px for titles, 11px for annotations. Never use font-size below 11.",
    "- For Chinese text labels longer than 6 characters, use <tspan> with dy to break into multiple lines, or shorten the label.",
    "- Ensure at least 8px gap between adjacent text elements to prevent overlap.",
    "",
    "## SVG Conventions",
    "",
