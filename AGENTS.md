...
- S mode SVG prompt requires a light background `<rect>` as the first element and explicit `fill` on all `<text>`. The CSS container `.visualSvgDiagram` uses light `#f8f9fa` background so diagrams remain readable even if the AI omits the background rect.
