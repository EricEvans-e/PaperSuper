...
- B-mode prompt treats the selected passage as PRIMARY source and the VisualSpec as reference-only (8 fields, not 12), so stale spec content from previous generations does not override the current PDF selection.
...
- AI-generated SVG principle diagrams are inserted with `dangerouslySetInnerHTML` only after `sanitizeSvg` removes scripts, event handlers, foreignObject, external links, and overlong payloads. Keep that sanitizer intact. `SVG_ALLOWED_ELEMENTS` must include `feOffset` and `feBlend` for complete filter chains; `SVG_EVENT_ATTR_RE` must not match `data-*` or `aria-*` prefixes (those are handled by the `startsWith` guard in the walk function).
- S mode SVG prompt requires a light background `<rect>` as the first element and explicit `fill` on all `<text>`. The CSS container `.visualSvgDiagram` uses light `#f8f9fa` background so diagrams remain readable even if the AI omits the background rect. Keep the prompt concise — focus on readability constraints only, do not prescribe specific colors, sizes, or marker IDs.
- `SVG_ALLOWED_ELEMENTS` includes animation elements (`animate`, `animateTransform`, `set`) and advanced filter primitives (`feTurbulence`, `feDisplacementMap`, `feComponentTransfer`, `feFuncR/G/B/A`, lighting elements). `SVG_ALLOWED_ATTRS` includes animation, text-layout, color-management, and lighting attributes. Keep these whitelists broad so the sanitizer does not strip valid SVG features.
- SVG CSS uses `max-width: 100%` (not `width: 100%`) so SVGs use their natural viewBox aspect ratio without being stretched.
