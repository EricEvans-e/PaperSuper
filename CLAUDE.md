...
- **S mode SVG supports Ctrl+wheel zoom** (0.3x–3x) and double-click reset. Toolbar shows zoom percentage, 1:1 reset, and expand/collapse toggle. SVG container uses light `#f8f9fa` background; prompt requires a background `<rect>` and explicit `fill` on all text elements.
